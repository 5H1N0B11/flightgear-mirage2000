...
var weaponRadarNames = {
	...
	# It is a list of folders inside ai/models that has weapons.
	"AGM65": nil,
	"AIM-54": nil,
	"aim-7": nil,
	"aim-9": nil,
...
var RadarRDY = {
	...
	update: func(tempCoord = nil, tempHeading = nil, tempPitch = nil) {
		...
		if (tempHeading == nil) {
			me.OurHdg = me.input.hdgReal.getValue();
		} else {
		...
		if (tempPitch == nil) {
			me.OurPitch = me.input.pitch.getValue();
		} else {
	...
	next_loop: func(index,factor) {
		...
		for(i=1; i < size(me.tgts_list); i = i + 1) {
			...
			if (me.tgts_list[number].get_display() == 1) {
				return number;
			}
		}
		return index;
	...
	next_Target_Index: func() {
		...
		if (size(me.tgts_list) > 0) {
			...
			} #If it require a focus, paint it
		...
		foreach(me.u;completeList) {
			...
			if (me.show == 1) {
				...
				if (me.u.get_model() == "missile_frigate" or me.u.get_model() == "fleet") {
					...
					me.danger = 40;
				} elsif (me.u.propNode.getName() == "SA-6") {
					me.danger = 15;
				} elsif (me.u.propNode.getName() == "ZSU-23-4M") {
					me.danger = 5;
