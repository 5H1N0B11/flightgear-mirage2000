...
# AP BUTTONS (AP: auto pilot - PA: Pilote Automatique)
var setAPMode = func(btn) {
...
var _updateNav = func() {
	...
	if(NAVSRC == "NAV1")
	{
		...
		_courseOffset("/instrumentation/nav[0]/radials/selected-deg");
	...
	elsif(NAVSRC == "NAV2")
	{
		...
		setprop("/autopilot/internal/gs-valid", gs);
		setprop("/autopilot/internal/nav-type", sgnl);
		_courseOffset("/instrumentation/nav[1]/radials/selected-deg");
	...
	elsif(NAVSRC == "TACAN")
	{
		if(getprop("/instrumentation/tacan/indicated-bearing-true-deg") != nil)
		{
			...
			_courseOffset("/instrumentation/tacan/indicated-bearing-true-deg");
...
# Tests PA-Limits
var _monitorAPErrors = func() {
	if (getprop("/fdm/jsbsim/fbw/gain-switch") != 0) {
		_setAPOff("AP-FBW-NOT-IN-FLIGHT");
	} else {
		...
		if (ralt < getprop("/autopilot/settings/minimums")) {
			_setAPOff("AP-<MINI-ALTITUDE");
		} else if (rlimit > 65 or rlimit < -65) {
			_setAPOff("AP-BANKLIMIT-FAIL");
		} else if (plimit > 30 or plimit < -30) {
			_setAPOff("AP-PITCHLIMIT-FAIL");
