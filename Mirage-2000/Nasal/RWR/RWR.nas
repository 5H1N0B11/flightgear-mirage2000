...
#TO do = update distance to RWR in fonction of the position on it : if vertical on 2D RWR is high, distance should be lower.
#find a trigonometric way to calculate the y position (2D RWR) as the real hud have around 45° of inclinaison.
#Make it happen for all non null radar properies
#Make null properties hidded



#var centerRWRx = (-0.07606 + 0.05357)/2;
#var centerRWRy = (-0.07327 +0.07327)/2;
#var centerRWRz = (-0.03237 +0.11536)/2;


#centerRWRx = centerRWRx+0.456-3.653;
#centerRWRy = centerRWRy;
#centerRWRz = centerRWRz+0.159-0.297;
#centerRWRz = 0.040;
...
var RWR = {
  ...
  new: func(placement)
  {
    var m = {
      parents: [RWR],
      canvas: canvas.new(RWR.canvas_settings)
    };
    m.viewPlacement = 480;
  ...
  update: func()
  {
    ...
    var Pilotx = getprop("sim/current-view/z-offset-m");
     var xCube = (centerRWRx - Pilotx)*(centerRWRx - Pilotx);
     var yCube = (centerRWRy - Piloty)*(centerRWRy - Piloty);
     var zCube = (centerRWRz - Pilotz)*(centerRWRz - Pilotz);
     
     var offsetZ = centerRWRz-Pilotz;
     
     #print("centerRWRx=" ~ centerRWRx ~ "centerRWRy=" ~ centerRWRy ~ "centerRWRz=" ~centerRWRz);
...
#  var hud_pilot = RWR.new({"node": "canvasRWR", "texture": "hud.png"});
#  hud_pilot.update();
#  var hud_copilot = RWR.new({"node": "verre2"});
#  hud_copilot.update();
#});
